...
#| label: dvs-2-ex-2
# Lab 4 Q8, modified
# Used ChatGPT to find high contrast and commonly considered color-blind friendly hex color codes.
ggplot(data = ca_childcare,
       mapping = aes(x = mhi_2018, y = mc_infant)) +
  geom_point(color = '#0072B2') +
  geom_smooth(color = '#E69F00', method = lm) +
...
#| label: dvs-6-ex-2
# Lab 9 Q8
# Revision: Modified red and green colors to be friendly for people with different color difficiencies. 
missing_vals |> 
  ...
  tab_style(list(cell_text(align = "center", weight = "bold")),
            locations = cells_column_labels()) |> 
  tab_style(style = list(cell_fill(color = "#D55E00"),
                         cell_text(color = "white")),
            locations = cells_body(columns = `Missing Values`,
                                   rows = `Missing Values` > 0)) |> 
  tab_style(style = list(cell_fill(color = "#009E73"),
                         cell_text(color = "white")),
            locations = cells_body(columns = `Missing Values`,
                                   rows = `Missing Values` == 0))
...
na_summary <- fish |> 
  ...
  mutate("Total NAs" = sum(c_across(everything()))
    )
na_summary
```
...
#| label: pe-1-map-1
# Lab 9 Q5
# Revision: Used map() and bind_rows() rather than map_dfr which is superseded
junior <- c(1,2,3,4)
senior <- c(5,6,7,8,9,10,11)

demographics_table <- evals |> 
  mutate(seniority = as.factor(seniority),
         Seniority = fct_collapse(.f = seniority,
                                  "Junior" = junior,
                                  "Senior" = senior),
         `Academic Degree` = fct_recode(academic_degree,
                                        "No Degree" = "no_dgr",
                                        "Masters" = "ma",
                                        "Doctorate" = "dr",
         ...
         Sex = fct_recode(sex,
                          "Female" = "female",
                          "Male" = "male"))|> 
  select(teacher_id, `Academic Degree` , Seniority , Sex) |>
  distinct(teacher_id, .keep_all = TRUE) |>
  ungroup() |> 
  select("Academic Degree" , Seniority , Sex) |> 
  map(~ {tibble(level = levels(.x),
...
rescale_column <- function(df, cols){
  if (!is.data.frame(df)) return(NA)
  df |> 
    mutate(across({{cols}}, ~ as.numeric(.x)),
...
#| label: pe-3-map-1
# Lab 9 Q3
# Revision: Used stringr function to ensure that the first letter in the Data Type column is capitalized.
surveys |> 
  map_chr(.f = ~ class(.x)) |> 
  ...
  arrange(`Data Type`) |> 
  kable(caption = "Datatypes for each column in surveys data for study on trout") |> 
...
mycifun <- function(beta0, beta1, n){
  ...
  ci_df <- broom::tidy(conf_int_lm,
            conf.int = TRUE,
            conf.level = 0.95) |>  
  filter(term != "(Intercept)") |> 
  mutate(cover = as.integer(conf.low <= beta1 & beta1 <= conf.high)) |> 
